...
func TestGetUsage(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 2:
			fmt.Fprintf(w, testPaginatedUsageResponse)
		case 3:
			fmt.Fprintf(w, testUsageResponse)
		}
	}))
	...
	if len(usage) != 2 {
		t.Errorf("Expected 2 records, got %d", len(usage))
	}
	...
	if len(usage) != 4 {
		t.Errorf("Expected 4 records, got %d", len(usage))
	}
}
...
`
const testPaginatedUsageResponse = `
{
   "first_page_uri": "/2010-04-01/Accounts/ACXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX/Usage/Records.json?Page=0&PageSize=50",
   "previous_page_uri": null, 
   "uri": "/2010-04-01/Accounts/ACXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX/Usage/Records.json", 
   ...
   "next_page_uri": "/2010-04-01/Accounts/ACXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX/Usage/Records.json?Category=calls&StartDate=2012-08-15&EndDate=2012-10-13",
   "page": 0
}
`
